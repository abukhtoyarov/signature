...
static size_t parseBlockSize(const variable_value& var) {
    ...
    if(m[3] == "")
        ...
        // kilobytes
        if(bs > 10 * 1024)
            throw runtime_error("block-size option must be in range from 512b to 10Mb.");
        mult = kb_mult;
    }
    else if(m[3] == "M" || m[3] == "m") {
        // megabytes
        ...
        throw runtime_error("Unhandled prefix: " + m[2].str() + " possible unit values: b, Kb, Mb; b - default" );
...
bool parse_command_line(int argc, const char** argv, Config& config) {
    try {
        options_description desc{
            "signature -i <input file> -o <output file> [-b <block size>(b|Kb|Mb)]\n\n"
            "Usage"
        };
        ...
        desc.add_options()
            ...
            ("block-size,b", value<string>()->default_value("1Mb"),
                "block size is integer value with unit (possible unit values: b, Kb, Mb; b - default)")
            ;
        ...
        if(vm.count("input"))
            config.inputFile = vm["input"].as<string>();

        if(vm.count("output"))
            config.outputFile = vm["output"].as<string>();
