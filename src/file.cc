...
void File::open(const string& file, Disposition d)
{
    hndl_ = CreateFile(
        ...
        d == Disposition::Read ? OPEN_EXISTING : CREATE_ALWAYS,
...
size_t File::read(Stream& stm, vector<char>& buf,
...
{
    fd_ = ::open(file.c_str(),
        d == Disposition::Read ? O_RDONLY : O_WRONLY | O_CREAT, 0666);
    if (fd_ == -1)
        throw runtime_error("Failed to open file " + file);
}

boost::asio::posix::stream_descriptor File::stream(io_context& io) const
{
    return { io, fd_ };
}
...
size_t File::read(Stream& stm, vector<char>& buf,
    boost::system::error_code& err)
{
    return ::boost::asio::read(stm, ::boost::asio::buffer(buf.data(), buf.size()),
        ::boost::asio::transfer_at_least(buf.size()), err);
}
...
size_t File::write(Stream& stm, string& buf, boost::system::error_code& err)
{
    return ::boost::asio::write(
        stm, ::boost::asio::buffer(buf.data(), buf.size()), err);
}
